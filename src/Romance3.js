...
function Romance3({page,id}) {
  const [movieData,setmovieData]=useState([])
  // function to fetch most popular movies from tmdb API
  useEffect(
          ()=>{
            fetch('https://api.themoviedb.org/3/discover/movie?api_key=4ba0c9d319e748dedd11b904098470a7&with_genres=10749&page=3')
            .then(res=>res.json())
        ...
        return <div className="AllMoviesContainer">
                  ...
                  <hr />
                    <div className="moviebtns">
                      ...
                      <button onClick={()=>page('/Romance3')} className='current'><h3>3</h3></button>
                      <button onClick={()=>page('/Romance4')} ><h3>4</h3></button>
                      <button onClick={()=>page('/Romance5')} ><h3>5</h3></button>
                      <div onClick={()=>page('/RomanceEnd')}><KeyboardDoubleArrowRightIcon/></div>
                    </div>
                    <footer>
                      <div className="links">
                          <h5>tems</h5>
