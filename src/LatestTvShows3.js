...
function LatestTvShows3() {
  const [movieData,setmovieData]=useState([])
  const [loading, setloading] = useState(true)
  // function to fetch most popular movies from tmdb API
  useEffect(
          ()=>{
            fetch('https://api.themoviedb.org/3/discover/tv?api_key=4ba0c9d319e748dedd11b904098470a7&language=en-US&sort_by=popularity.desc&page=1&timezone=America%2FNew_York&include_null_first_air_dates=false&with_watch_monetization_types=flatrate&with_status=0&with_type=0&page=3')
            .then(res=>res.json())
            .then(data=>{
                  ...
                  <hr />
                    {
                      movieData.length !==0  &&
                          <div>
                            ...
                            <Link to={`/LatestTvShows3`}><button  className='current' ><h3>3</h3></button></Link>
