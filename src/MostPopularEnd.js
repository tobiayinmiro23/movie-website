...
function MostPopularEnd({page,id}) {
  const [movieData,setmovieData]=useState([])
  // function to fetch most popular movies from tmdb API
  useEffect(
          ()=>{
            fetch('https://api.themoviedb.org/3/movie/popular?api_key=4ba0c9d319e748dedd11b904098470a7&language=en-US&page=6&sort_by=popularity.desc')
            .then(res=>res.json())
        ...
        return <div className="AllMoviesContainer">
                  ...
                  <hr />
                    ...
                    <button  onClick={()=>page('/MostPopular')} ><h3>1</h3></button>
                      ...
                      <button onClick={()=>page('/MostPopular4')} ><h3>4</h3></button>
                      <button onClick={()=>page('/MostPopular5')} ><h3>5</h3></button>
                      <div onClick={()=>page('/MostPopularEnd')} className='current'><KeyboardDoubleArrowRightIcon/></div>
                    </div>
                    <footer>
                      <div className="links">
                          <h5>tems</h5>
