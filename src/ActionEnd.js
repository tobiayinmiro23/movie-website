...
function ActionEnd({page,id}) {
  const [movieData,setmovieData]=useState([])
  // function to fetch most popular movies from tmdb API
  useEffect(
          ()=>{
            fetch('https://api.themoviedb.org/3/discover/movie?api_key=4ba0c9d319e748dedd11b904098470a7&with_genres=28&page=6')
            .then(res=>res.json())
  ...
  const ShowMovie=(param)=>{
    ...
    page('/Movie')
        ...
        return <div className="AllMoviesContainer">
                  <header>
                      <div className="img"><img src="/image/arrow.png" alt="" /></div>
                    ...
                    {movieData.map(data=>{
                      return(
                              <div key={data.id}  className="movieContainer">
                                  ...
                                  <img key={data.id}  src={`https://image.tmdb.org/t/p/w500${data.poster_path}`} alt="movie img" />
                                  </div>
                  ...
                  <hr />
                    <div className="moviebtns">
                      ...
                      <button onClick={()=>page('/Action5')}><h3>5</h3></button>
                      <div onClick={()=>page('/ActionEnd')} className='current' ><KeyboardDoubleArrowRightIcon/></div>
                    </div>
                    <footer>
                      <div className="links">
                          <h5>tems</h5>
