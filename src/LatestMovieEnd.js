...
function LatestMovieEnd({page,id}) {
  const [movieData,setmovieData]=useState([])
  // function to fetch most popular movies from tmdb API
  useEffect(
          ()=>{
            fetch('https://api.themoviedb.org/3/discover/movie?api_key=4ba0c9d319e748dedd11b904098470a7&language=en-US&sort_by=popularity.desc&include_adult=false&include_video=false&page=1&with_watch_monetization_types=flatrate&page=6')
            .then(res=>res.json())
    ...
    page('/Movie')
        ...
        return <div className="AllMoviesContainer">
                  ...
                  <hr />
                    ...
                    <button  onClick={()=>page('/LatestMovie')}><h3>1</h3></button>
                      < button onClick={()=>page('/LatestMovie2')} ><h3>2</h3></button>
                      <button onClick={()=>page('/LatestMovie3')} ><h3>3</h3></button>
                      <button onClick={()=>page('/LatestMovie4')} ><h3>4</h3></button>
                      <button onClick={()=>page('/LatestMovie5')}><h3>5</h3></button>
                      <div onClick={()=>page('/LatestMovieEnd')} className='current'><KeyboardDoubleArrowRightIcon/></div>
                    </div>
                    <footer>
                      <div className="links">
                          <h5>tems</h5>
