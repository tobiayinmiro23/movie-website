...
function Comedy5({page,id}) {
  const [movieData,setmovieData]=useState([])
  // function to fetch most popular movies from tmdb API
  useEffect(
          ()=>{
            fetch('https://api.themoviedb.org/3/discover/movie?api_key=4ba0c9d319e748dedd11b904098470a7&with_genres=35&page=5')
            .then(res=>res.json())
        ...
        return <div className="AllMoviesContainer">
                  ...
                  <hr />
                    ...
                    <button  onClick={()=>page('/Comedy')}><h3>1</h3></button>
                      ...
                      <button onClick={()=>page('/Comedy5')} className='current'><h3>5</h3></button>
                      <div onClick={()=>page('/ComedyEnd')}><KeyboardDoubleArrowRightIcon/></div>
                    </div>
                    <footer>
                      <div className="links">
                          <h5>tems</h5>
